...
<script>
	...
	export default {
		...
		methods: {
			...
			loginThenJquery(obj){ //登录之后在执行查询
				let scanPerson = this.scanParamsData[1]
				let parsonId = JSON.parse(uni.getStorageSync('userInfo')).personId || -1;
				console.log(this.scanParamsData)
				if(scanPerson == parsonId){
				let obj1 = {type: 4,params: {QUERY_ID: 2,MODE: 0,BILL_ID: this.scanParamsData[0], PartnerId: this.scanParamsData[1]}}
