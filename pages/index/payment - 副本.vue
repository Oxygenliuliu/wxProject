	...
	<view class="con-conent">
						...
						<view class="daily-list" v-for="(item,index) in it.child" :key="index">
							<view class="daily-listinfo">
								<view class="daily-img">
									<img :src="item.img" onerror="javascript:this.src='../../static/pngs/unExist.png'" alt="">
								</view>
							...
							<view class="daily-head">
								...
								<view class="daily-contacts w-text-over warning">明细总价: {{item.BILL_LEFTMONEY|priceFormat(2,'￥',true)}}</view>
							</view>
						</view>
					</view>
				</view>
		...
		<view class="recfooter">
			<view class="pricebtn">
				...
				<view class="right1">
					<text class="right w-btn-info" @tap="canle()">去往订单页面</text>
		...
		methods: {
			...
			loginThenJquery(){ //登录之后在执行查询
				...
				if(options.q != undefined){ //扫码进来
					...
					// obj = {type: 3,params:{QUERY_ID: 2,MODE: 0,BILL_ID: "182454891578167296",PartnerId: '1757112'}};
			...
			async queryData(params = this.params){ //查询数据
				...
				this.tipShow = this.dailyList.length ==0?true:false
				this.appendCheckBox();
			},
				...
				return new Promise((resolve,reject)=>{
					wx.login({
						success: res => {
							var code = res.code;
							 uni.request({
								url:'https://proxy.fydlsoft.com' + '/unifiedorder',
								method: 'POST',
								data:{
									token: l_md5(code+'FYDL'),
									code: code,
