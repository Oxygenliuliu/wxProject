						...
						<view class="daily-list" v-for="(item,index1) in it.child" :key="index1">
							<view class="daily-listinfo">
								<view class="daily-img" @tap.stop="previewImage(it.child,index1)">
									<image :src="'https://cloud.fydlsoft.com/transfer?REQUEST_URI=Request_Path=' + item.img" @error="defaultImg(index,index1)" alt=""></image>
								</view>
								<view class="daily-coninfo">
									<view class="daily-coninfoone">
										<view class="daily-conleft w-text-over">项目名称: {{it.BILLDETAIL_FILENAME || ''}}</view>
									</view>
									<view class="daily-coninfoone">
	...
	export default {
			...
			if(token){
				...
				try{
					if(options.q != undefined){
						...
						this.setNetInfo(str)
					}else if(options.q == undefined){ //小程序分享
						this.$store.commit('setParams',options);
						// this.setNetInfo(options.data)
						let csAppData = JSON.parse(options.data)
						let serverInfo = csAppData.params
		...
		methods: {
			...
			setNetInfo(data){ //分享进来预先设置dbname和url
				let newArr = data.replace(/\"/g,'').split(',');
				let index = newArr[3].indexOf(':')
				let scUrl = newArr[3].substr(0,index)
				let scPort = newArr[3].substr(index + 1,newArr.length)
				this.$store.commit('changeDbname',newArr[2]);
				this.$store.commit('setDbnameCode',newArr[2]);
				this.$store.commit('changePort',scPort);
				this.$store.commit('changeUrl',scUrl);
			},
			loginThenJquery(){ //登录之后在执行查询
				...
				if(options.q != undefined){ //扫码进来
					...
					if(newArr.length == 4){
						...
						let shareParsonId1 = newArr[1] //客户ID
						if(shareParsonId1 == parsonId){
								let obj1 = {type: 4,params: {QUERY_ID: 2,MODE: 0,BILL_ID: newArr[0], PartnerId: shareParsonId1}}
								this.someInit(obj1);//初始化
								this.queryData();
						}else{
							uni.showToast({title: '不是您的专属分享链接',icon: 'none', duration:3000})
						}
					}else{
					...
					if(options.data.indexOf('type') != -1){ //分享二维码
						...
						let newArr = options.data.replace(/\"/g,'').split(',');
						let shareParsonId1 = newArr[1] //客户ID
						if(parsonId != shareParsonId1){
							uni.showToast({title: '不是您的专属分享链接',icon: 'none', duration:3000})
						}else{
							let obj1 = {type: 4,params: {QUERY_ID: 2,MODE: 0,BILL_ID: newArr[0], PartnerId: shareParsonId1}}
							this.someInit(obj1);//初始化
