<template>
	<view class="con-conent">
		...
		<swiper class="swiper-box" :current='tabIndex' :duration="300"  :style="{ 'height':scrollHeight }" @change="ontabchange">
			...
			<!-- #ifndef MP-WEIXIN -->
			<l-refresh ref="pullScroll" :backTop="true" :pullDown="pullDown" :pullUp="loadData">
			<!-- #endif -->
			<!-- #ifdef MP-WEIXIN -->
			<l-refresh ref="pullScroll" :backTop="true" :pullDown="pullDown" :pullUp="loadData">
			<!-- #endif -->
						...
						<view class="daily-list" v-for="(item,index) in it.child" :key="index">
							<view class="daily-listinfo">
								<view class="daily-img" @tap.stop="previewImage(it.child,index2)">
									<image :src="'https://cloud.fydlsoft.com/transfer?REQUEST_URI=Request_Path=' + item.img" @error="defaultImg(index2,index)" alt=""></image>
								</view>
...
<script>
	import lRefresh from '@/components/uni-refresh/l_refresh.vue'
	...
	export default {
		...
		data() {
			return {
				...
				swiperQuery: false,
				paymoney: '0',//微信支付金额
				isLoadTrue: false,//是否加载完成
			}
		},
		methods: {
			//组件下拉上滑
			pullDown(pullScroll){
				this.params.Status = this.tabIndex || 0
				this.params.PAGE_INDEX = 0;
				this.queryData(pullScroll);
			},
			loadData(pullScroll){
				if(this.isLoadTrue){
					return
				...
				console.log(this.nowInfo.nowIndex)
				if(this.nowInfo.maxSize >= this.nowInfo.nowIndex || this.nowInfo.maxSize == -1){
					this.addPageInfo();
					this.wrquest(this.params).then(res=>{
						if(res.length < 8){
							pullRefresh.finish();
						}else{
							pullScroll.success();
						}
						this.setDisplayData(res);
					}).catch(err=> {
						pullScroll.empty()
					}).finally(()=> {
			...
			wrquest(params = this.params){//接口调用
				...
				return new Promise((resolv,reject)=>{
					uni.request({
						...
						fail:(res)=> {
							console.log(res)
							reject(uni.showToast({title: '网络异常',icon: 'none'}))
						},
						complete:(res)=> {
			...
			async queryData(pullScroll){ //查询数据
				this.wrquest(this.params)
				.then(res => {
					this.dailyList = this.createDoubleArr(res);
